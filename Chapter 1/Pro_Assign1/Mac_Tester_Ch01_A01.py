...
cmd = subprocess.run(["python3", "Ch01_A01_favorite_quote.py"], stdout=PIPE, stderr=STDOUT,)
...
cmd = subprocess.run(["python3", "Ch01_A01_Go_SUU.py"], stdout=PIPE, stderr=STDOUT,)
stdout = cmd.stdout.decode()  # bytes => str
info = stdout
info = info.replace("\r","")
if (
